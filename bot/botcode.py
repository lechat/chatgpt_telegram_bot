...
import bot.openai_utils as openai_utils


# setup
db = Database.load_db(Config())
...
async def start_handle(update: Update, context: CallbackContext):
    ...
    reply_text = (
        "Hi! I'm <b>ChatGPT</b> bot implemented with GPT-3.5 OpenAI API 🤖\n\n"
    )
    ...
    async with user_semaphores[user_id]:
        ...
        try:
            ...
            chatgpt_instance = openai_utils.ChatGPT(
                Config(), use_chatgpt_api=config.use_chatgpt_api
            )
            ...
            async for gen_item in gen:
                ...
                if (
                    i == 0
                ):  # send first message (then it'll be edited if message streaming is enabled)
                    try:
                    ...
                    if (
                        abs(len(answer) - len(prev_answer)) < 100
                        and status != "finished"
                    ):
                        continue
...
async def voice_message_handle(update: Update, context: CallbackContext):
    ...
    price_per_1000_tokens = (
        ...
        else config.gpt_price_per_1000_tokens
    )
...
async def show_chat_modes_handle(update: Update, context: CallbackContext):
    ...
    await update.message.reply_text(
        "Select chat mode:", reply_markup=reply_markup
    )
...
def run_bot() -> None:
    ...
    application.add_handler(
        CommandHandler("retry", retry_handle, filters=user_filter)
    )
    application.add_handler(
