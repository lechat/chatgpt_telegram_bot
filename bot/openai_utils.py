...
class ChatGPT:
    def __init__(self, config, use_chatgpt_api=True):
        self.config = config
        self.use_chatgpt_api = use_chatgpt_api
        self.CHAT_MODES = config.chat_modes
        openai.api_key = config.openai_api_key

    async def send_message(
        self, message, dialog_messages=[], chat_mode="assistant"
    ):
        if chat_mode not in self.CHAT_MODES.keys():
    ...
    def _generate_prompt(self, message, dialog_messages, chat_mode):
        prompt = self.CHAT_MODES[chat_mode]["prompt_start"]
        prompt += "\n\n"
    ...
    def _generate_prompt_messages_for_chatgpt_api(
        self, message, dialog_messages, chat_mode
    ):
        prompt = self.CHAT_MODES[chat_mode]["prompt_start"]
    ...
    def _count_tokens_for_chatgpt(
        self, prompt_messages, answer, model="gpt-3.5-turbo"
    ):
    ...
    def _count_tokens_for_gpt(self, prompt, answer, model="text-davinci-003"):
        encoding = tiktoken.encoding_for_model(model)
        n_tokens = (
            len(encoding.encode(prompt)) + len(encoding.encode(answer)) + 1
        )
        return n_tokens
